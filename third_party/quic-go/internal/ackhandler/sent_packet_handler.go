...
// CongestionControlFactory constructs a congestion controller for a connection.
// qlogger may be nil. If a nil CongestionControlFactory is supplied to
// NewSentPacketHandler, it defaults to constructing a Cubic sender.
type CongestionControlFactory func(rttStats *utils.RTTStats, connStats *utils.ConnectionStats, initialMaxDatagramSize protocol.ByteCount, qlogger qlogwriter.Recorder) congestion.SendAlgorithmWithDebugInfos

func defaultCongestionControlFactory(rttStats *utils.RTTStats, connStats *utils.ConnectionStats, initialMaxDatagramSize protocol.ByteCount, qlogger qlogwriter.Recorder) congestion.SendAlgorithmWithDebugInfos {
...
func NewSentPacketHandler(
	...
	qlogger qlogwriter.Recorder,
	logger utils.Logger,
	congestionControlFactory CongestionControlFactory, // if nil, defaults to Cubic
) SentPacketHandler {
	if congestionControlFactory == nil {
		congestionControlFactory = defaultCongestionControlFactory
	}
	...
	h := &sentPacketHandler{
		...
		rttStats:                       rttStats,
		connStats:                      connStats,
		congestion:                     congestionControlFactory(rttStats, connStats, initialMaxDatagramSize, qlogger),
		congestionFactory:              congestionControlFactory,
...
func (h *sentPacketHandler) MigratedPath(now monotime.Time, initialMaxDatagramSize protocol.ByteCount) {
	...
	h.congestion = h.congestionFactory(h.rttStats, h.connStats, initialMaxDatagramSize, h.qlogger)
	h.setLossDetectionTimer(now)
}

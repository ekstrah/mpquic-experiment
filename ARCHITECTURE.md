...
Path 0 is special: it's the only path that describes the whole transfer
...
```
  [8B  Seq]        uint64  — chunk's position in the logical chunk sequence
  [8B  Offset]     uint64  — byte offset into the payload
...
chunks came before it or how many are still to come, which matters once
this moves to continuous/unbounded streaming (see README's "Known
...
initialized the session (`sess.start`), because only the control header
knows the total size / chunk size needed to construct the `Reassembler`.

A `sync.WaitGroup` sized to `ctrl.NumPaths` tracks how many paths have
finished; the last one to call `sess.pathDone` triggers `sess.finalize()`,
which does the integrity determination and writes the three server-side
result files.
...
chunk that fails its checksum is logged, counted (`PathStats.ChunksCorrupted`),
and **not** handed to the `Reassembler` — under a redundant scheduler, a
good copy might still arrive on another path and get counted instead.

The server's authoritative `integrity_ok` is:
...
at least once*. The client-side result always reports `integrity_ok=n/a` —
it has no way to observe what the server actually reassembled.

This replaced an earlier design (one SHA-256 over the whole payload, sent
in the `ControlHeader` *before* any data, checked once at the very end).
That design required knowing the total payload upfront and could only ever
report one pass/fail verdict for the entire transfer. Per-chunk checksums
were a deliberate prerequisite for eventually supporting continuous,
unbounded streaming with variable burst sizes — see README's "Known
limitations / next steps".
...
- Shared state within a session is protected by: `serverSession.mu`
  (control header / reassembler / progress printer setup, guarded by the
  `ready` channel so no path proceeds before path 0's header arrives),
  `serverSession.pathMu` (appending to `pathResults`), and
  `Reassembler.mu` (chunk placement — the same reassembler instance is
  written to concurrently by every path's goroutine).
